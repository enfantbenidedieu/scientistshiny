...
from scientisttools import PCA,fviz_pca_ind,fviz_pca_var,fviz_eig,fviz_contrib,fviz_cos2,dimdesc
...
class PCAshiny(Base):
    ...
    Description
    -----------
    Performs Principal Component Analysis (PCA) with supplementary elements (individuals and/or quantitative variables and/or qualitative variables) on a Shiny for Python application. Allows to change PCA graphical parameters. Graphics can be downloaded in png, jpg and pdf.
    ...
    Parameters
    ----------
    `model`: a pandas dataframe with n rows (individuals) and p columns (variables) or an object of class PCA (a PCA result from scientisttools).

    Returns
    -------
    `Graphs` : a tab containing the individuals factor map and the variables factor map

    `Values` : a tab containing the eigenvalue, the results for the variables, the results for the individuals, the results for the supplementary elements (individuals and/or quantitative variables and/qualitative variables).
    ...
    def __init__(self,model=None):
        ...
        if isinstance(model,pd.DataFrame):        
            ...
            # Fit the PCA with scientisttools
            model = PCA(quali_sup=quali_sup).fit(model)
        ...
        app_ui = ui.page_fluid(
            ...
            header(title="Analyse en Composantes Principales",model_name="PCA"),
            ui.page_sidebar(
                ui.sidebar(
                    ui.panel_well(
                        ...
                        ui.panel_conditional("input.fviz_ind_var === 'fviz_ind'",
                            ...
                            ui.panel_conditional("input.ind_text_color === 'var_quant'",ui.input_select(id="ind_text_var_quant_color",label="choix de la variable",choices={x:x for x in var_labels},selected=var_labels[0],multiple=False,width="100%")),
                ...
                ui.navset_card_tab(
                    ...
                    ui.nav_panel("Valeurs",
                        ...
                        ui.panel_conditional("input.value_choice === 'var_res'",
                            ...
                            ui.panel_conditional("input.var_choice === 'contrib'",panel_conditional2(text="var",name="contrib")),
                            ui.panel_conditional("input.var_choice === 'cos2'",panel_conditional2(text="var",name="cos2"))
                        ),
        ...
        def server(input:Inputs, output:Outputs, session:Session):
            ...
            # Individual - PCA
            @render.plot(alt="Individuals - PCA")
            def fviz_ind_plot():
            ...
            def _():
                graph_modal_show(text="var",name="contrib",max_axis=model.call_["n_components"])
            ...
            def _():
                graph_modal_show(text="var",name="cos2",max_axis=model.call_["n_components"])
            ...
            @reactive.Calc
            def data():
                data = model.call_["Xtot"].loc[:,var_labels].astype("float")
